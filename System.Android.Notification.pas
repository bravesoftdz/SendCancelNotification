...
uses
  System.Notification, Androidapi.JNI.GraphicsContentViewText, System.Classes, System.SysUtils, System.DateUtils, System.Messaging, System.TimeSpan,
  ...
  public
    ...
    function GetAllNotificationsNamesNonFiltered: TStringList;
    function GetAllNotificationsNamesJStringToString: String;
  end;
...
function TNotificationCenterAndroid.CreateNativeNotification(const ANotification: TNotification): JNotification;
  ...
  begin
    ...
    Result := TJPendingIntent.JavaClass.getActivity(TAndroidHelper.Context, TGeneratorUniqueID.GenerateID, Intent, 0);
  end;
...
begin
  ...
  NotificationBuilder := NotificationBuilder.setDefaults(TJNotification.JavaClass.DEFAULT_VIBRATE or TJNotification.JavaClass.DEFAULT_LIGHTS);
...
begin
  ...
  begin
    ...
    NotificationsList := GetAllNotificationsNamesNonFiltered;
    try
...
function TAndroidPreferenceAdapter.GetAllNotificationsNamesJStringToString: String;
var
  NotificationsStr: JString;
begin
  result := '';
  NotificationsStr := FPreference.getString(TJNotificationAlarm.JavaClass.SETTINGS_NOTIFICATION_IDS, nil);
  result := JStringToString(NotificationsStr);
end;
    ...
    try
      ...
      for I := pos('=',sRemaining,1) to length(sRemaining) -1 do begin
        if pos( sRemaining[i], validChars ) > 0 then begin
          ...
          //NotificationsForm.mmo1.Lines.Add('('+ i.ToString +'):"'+sRemaining[i]+'" is valid' ) ;
        end else begin
